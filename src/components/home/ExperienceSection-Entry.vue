<template>
  <li
    class="before: before:bg-primary-900 relative pb-8 before:absolute before:left-[-35px] before:top-[6px] before:z-[1] before:h-4 before:w-4 before:rounded-full"
  >
    <article>
      <h1 class="open-sans text-primary-900 text-xl font-bold">
        {{ props.position }}
      </h1>
      ...
      <span class="block">
        <component
          ...
          :class="
            props.url == '#' ? 'cursor-default' : 'hover: hover:text-primary-700 cursor-pointer'
          "
          target="_blank"
